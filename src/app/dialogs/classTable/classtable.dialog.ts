...
export class ClassTableDialogComponent implements OnInit {
  ...
  handleClassTableDataResponse(res: ClassTable) {
    ...
    this.dataSource = new MatTableDataSource(this.currentPageData);
  }
  }
  ...
  onClickSaveExcel() {
    this._tableService.saveCSV$(this.data.uri,this.attributes.value, this.uriDisplayNames).subscribe((res: string)  => {
