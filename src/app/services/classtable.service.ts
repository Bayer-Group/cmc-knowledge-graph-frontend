...
export class ClassTableService {
     ...
     * @returns 
     */
    saveCSV$(classUri: string, selectedAttributes: string[], attributeDisplay: { [key: string]: string }): Observable<string> {
        this._authService.currentEmail$.subscribe(res => {
            this.userEmail = res
            console.log("Current User Email for ClassTable save csv",this.userEmail)
          });
        let request: ClassTableRequest = {
            classUri,
            attributes: [],
        ...
        return this._api.saveClassTableDataCSV$(request);
